# Prompt : 레프 톨스토이 『인생론(On Life)』 장별 분석 및 실천 정리

레프 톨스토이(Leo Tolstoy)의 『인생론(On Life)』을 깊이 있게 분석하는 **삶의 의미·자기 훈련 독서 코치**입니다.
아래 조건에 따라 책의 각 장(Chapter)을 정리해 주세요.

※ **책 식별 주의**: 이 책은 톨스토이가 1886\~1887년에 쓴 **철학·종교 논고 『인생론(О жизни, On Life)』**이다(1886년 여름 영지에서 부상으로 요양하던 중 집필 시작, 1887년 완성, 러시아 검열로 국외 선(先)출간 후 1891년 무삭제판). 한국에 『톨스토이 인생론』이라는 제목으로 흔히 유통되는 책이 이 책이 아니라, 톨스토이가 1884년부터 15년에 걸쳐 편집한 **날짜별 잠언·명상집**(러시아어 원제 『Круг чтения(독서의 순환)』, 영역 『A Calendar of Wisdom』)을 가리키는 경우도 많으므로 혼동하지 않는다. 또한 이 책은 알랭 드 보통의 『Status Anxiety』(지위 인생론)와도 전혀 다른 책이니, 능력주의·속물근성·보헤미아 같은 드 보통의 개념을 이 책 분석에 끌어오지 않는다. 자세한 서지 정보와 실제 목차는 같은 폴더의 `On-Life.md`(책 개요 문서)를 참조한다.
...
※ 『인생론』은 소설도, 날짜별 잠언집도 아니라 **서문 → 본문 35개 장(하나의 논증이 처음부터 끝까지 이어짐) → 결론 → 부록 3편**으로 구성된 연속된 철학 논고다. 각 장은 짧지만(대개 몇 쪽 분량), 앞 장의 결론이 다음 장의 전제가 되는 방식으로 긴밀하게 이어진다. 원문의 장 제목은 대부분 한 문장 전체로 되어 있어 파일명으로 쓰기에는 너무 길므로, 아래 표의 "파일명 슬러그"를 그대로 사용한다.
...
참고 — 『인생론』의 장 구성 (저본: The Free Age Press 영역 완역본, archive.org `onlife188800tolsrich`):
...
> 이 장은 『인생론』 전체 서문+35개 장+결론+부록 중 {O}번째(원문 로마 숫자 {N})에 해당하며, {이 장이 전체 논증에서 담당하는 역할 — 예: 근본 모순의 제시 / 이성적 의식의 요구 확립 / 사랑의 정의 / 죽음의 재해석 등}을 다룬다.
...
* 단어/표현 — 뜻, 본문에서의 쓰임 (예: On Life, O zhizni, animal individuality, reasonable consciousness, the Pharisees and the Scribes, the superstition of death 등)
